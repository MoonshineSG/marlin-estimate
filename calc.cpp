...
static bool relative_mode = false; // Determines Absolute or Relative
                                   // Coordinates
...
static char *strchr_pointer; // just a pointer to find chars in the cmd string
                             // like X, Y, Z, E, etc
...
// void plan_buffer_line(const double &x, const float &y, const float &z, const
// float &e, float feed_rate, const uint8_t &extruder);
...
void manage_inactivity2() {
  ...
  if (block != NULL) {
    ...
    //block->final_rate, block->acceleration_st);
    // calculate block len
